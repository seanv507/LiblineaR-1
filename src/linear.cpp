...
void Solver_MCSVM_CS::Solve(double *w)
{
	...
	while(iter < max_iter)
	{
		...
		{
			int j = rand_between(i,active_size); // i+rand()%(active_size-i);
...
void solve_l2r_lr_dual(const problem *prob, double *w, double eps, double Cp, double Cn)
{
	...
	while (iter < max_iter)
	{
		for (i=0; i<l; i++)
		{
			int j = rand_between(i,l); //i+rand()%(l-i);
			swap(index[i], index[j]);
		}
		int newton_iter = 0;
		double Gmax = 0;
	...
	double Cp, double Cn)
{
	...
	while(iter < max_iter)
	{
		...
		{
			int i = rand_between(j,active_size); //j+rand()%(active_size-j);
			swap(index[i], index[j]);
		}
	...
	double Cp, double Cn)
{
	...
	while(newton_iter < max_newton_iter)
	{
		...
		while(iter < max_iter)
		{
			...
			{
				int i = rand_between(j,QP_active_size); //j+rand()%(QP_active_size-j);
				swap(index[i], index[j]);
			}
...
extern "C" inline double get_w_value(const struct model *model_, int idx, int label_idx) 
{
	int nr_class = model_->nr_class;
...
extern "C" double get_decfun_coef(const struct model *model_, int feat_idx, int label_idx)
{
...
extern "C" double get_decfun_bias(const struct model *model_, int label_idx)
{
...
extern "C" int check_regression_model(const struct model *model_)
{
